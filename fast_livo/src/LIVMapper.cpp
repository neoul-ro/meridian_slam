...
void LIVMapper::readParameters(rclcpp::Node::SharedPtr &node)
{
  ...
  try_declare.template operator()<int>("preprocess.lidar_type", VELO16);
...
void LIVMapper::initializeComponents(rclcpp::Node::SharedPtr &node) 
{
  ...
  // Upstream hardcoded 0.0001 here and never read imu.b_gyr_cov / imu.b_acc_cov,
  // which pinned the bias random walk regardless of the measurement covariances.
  // With Allan-measured gyr_cov (5.9e-7) that left the bias noise 169x LARGER
  // than the measurement noise -- inverted from upstream's 3000x the other way --
  // so the filter charged every attitude error to gyro bias and ran away under
  // rotation. Reading the yaml lets both come from the same Allan measurement.
...
void LIVMapper::initializeSubscribersAndPublishers(rclcpp::Node::SharedPtr &node, image_transport::ImageTransport &it_)
{
  ...
  // The AVIA branch was the only thing pulling in livox_ros_driver2, which then
  // had to be present in the workspace for this package to configure at all --
  // on a rig that has never had a Livox on it. Every lidar_type this fork
  // actually runs speaks sensor_msgs/PointCloud2. Fail loudly rather than
  // silently subscribing with the wrong type if a config still asks for AVIA.
  if (p_pre->lidar_type == AVIA)
  {
    RCLCPP_FATAL(this->node->get_logger(),
                 "preprocess.lidar_type 1 (AVIA) needs livox_ros_driver2, which this build "
                 "no longer depends on. Use 2 (VELO16) or another PointCloud2 type.");
    throw std::runtime_error("AVIA lidar_type is not supported in this build");
  }
  ...
  // /pose used to be published here (upstream's /mavros/vision_pose/pose, the
  // body pose a flight controller wants). The Meridian pipeline wants the pose
  // of base_link, and base_link is not a frame this node knows: the mount
  // offsets live in slam.launch.py and the URDF. meridian_slam_bringup's
  // odom_tf_relay already applies them to build map->base_link, so it publishes
  // /pose now and the geometry stays in one place. The body pose is still on
  // /aft_mapped_to_init, which is what odom_tf_relay consumes.
...
void LIVMapper::handleVIO() 
{
  ...
  // Silenced 2026-08-10 (log-volume backpressure).
...
{    
  ...
  *pcl_w_wait_pub = *laserCloudWorld;
  ...
  // 2026-08-10: this 12-line table printed every LIO frame -- 94,688 of the
  // 445,334 lines logged in a 7-minute run. `ros2 launch` relays child stdout
  // on a single thread, and when that thread saturates the pipe fills and the
  // *sensor drivers* block in write() and drop frames. Silenced, not deleted.
...
void LIVMapper::imu_cbk(const sensor_msgs::msg::Imu::ConstSharedPtr &msg_in)
{
  ...
  // Silenced 2026-08-10: fires at the full 100 Hz IMU rate (43,513 lines in
  // 7 min). See the LIO timing table above for why the log volume matters.
  ...
  {
    // Upstream dropped the message here WITHOUT updating last_timestamp_imu,
    // so a single >0.2s gap (driver reconnect, clock step) rejected every
    // following message and stalled SLAM forever. Accept the new timebase
    // and drop only the stale history instead.
    RCLCPP_WARN(this->node->get_logger(), "imu time stamp Jumps %0.4lf seconds — resetting IMU stream\n", timestamp - last_timestamp_imu);
    imu_buffer.clear();
  }
  ...
  // Silenced 2026-08-10: also 100 Hz, and endl flushes on every line.
...
void LIVMapper::img_cbk(const sensor_msgs::msg::Image::ConstSharedPtr &msg_in)
{
  ...
  double msg_header_time = stamp2Sec(msg->header.stamp) + img_time_offset;
  if (abs(msg_header_time - last_timestamp_img) < 0.001) return;
  // Silenced 2026-08-10 (fires per camera frame, 30 Hz).
...
void LIVMapper::publish_frame_world(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudFullRes, VIOManagerPtr vio_manager)
{
  ...
  // Skip empty frames: they break decay-time accumulation in Foxglove/RViz.
  if (laserCloudmsg.width * laserCloudmsg.height > 0) pubLaserCloudFullRes->publish(laserCloudmsg);
...
void LIVMapper::publish_odometry(const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr &pubOdomAftMapped)
{
  ...
  // The measurement time, not the publish time. This stamp reaches consumers
  // twice over: odom_tf_relay copies it onto map->base_link and onto /pose.
  // get_clock()->now() here is the moment the LIO update finished, a median
  // 36.6 ms after the sweep the state belongs to (measured over 1116 poses in
  // 20260823_180259) -- enough that a consumer pairing poses to camera frames
  // within a few ms matches almost nothing.
  ...
  br->sendTransform(geometry_msgs::msg::TransformStamped(createTransformStamped(transform, odomAftMapped.header.stamp, "map", "aft_mapped")));
